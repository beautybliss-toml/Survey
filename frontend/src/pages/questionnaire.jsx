...
function Questionnaire() {
  ...
  const getData = () => {
    axios
      ...
      .then(function (response) {
        let temp = response.data.questions.filter(
          (item) => item.group_id === store_group_id
        );
  ...
  useEffect(() => {
    axios
      .post(`${config.server_url}/store/getStoreInfo`, { store_url_name })
      .then(function (response) {
  ...
  useEffect(() => {
    ...
    questionStatus.Congratlation1.display,
    questionStatus.Congratlation2.display,
  ]);
  //
  ...
  const changeQuestionStatus2 = (value, cur_id, nextQuestion) => {
    ...
    if (
      nextQuestion === "最後のアンケート1" ||
      nextQuestion === "最後のアンケート2"
    ) {
      setQuestions([...temp]);
  ...
  const nextQuestion1 = (questionId, require) => {
    if (questions[questionId].answer === -1 && require) {
      ...
      questions[questionId].connect === "最後のアンケート1" ||
      questions[questionId].connect === "最後のアンケート2"
    ) {
      let temp = questions;
  ...
  const nextQuestion = (questionId) => {
    if (questions[questionId].answer === -1) {
      ...
      for (let i = 0; i < arr.length; i++)
        if (arr[i] === temp[questionId].answer)
          nextId = temp[questionId].select2.split(",")[i];
  ...
  const sendAnswer = () => {
    ...
    let st = "";
    if (questionStatus.Congratlation1.display)
      st = questionStatus.Congratlation1.previousId;
    ...
    temp.sort(
      (a, b) =>
        parseInt(a.question_no.slice(1, a.question_no.length)) -
        parseInt(b.question_no.slice(1, b.question_no.length))
    );
    ...
    for (let i = 0; i < temp.length; i++) {
      question_nos +=
        i === temp.length - 1 ? temp[i].question_no : temp[i].question_no + ",";
      question_names +=
        i === temp.length - 1
          ? temp[i].question_name
          : temp[i].question_name + ",";
    ...
    axios
      .post(`${config.server_url}/review/create`, temp)
      .then(function (response) {
              ...
              <main key={idx} className="flex flex-col items-center">
                <div className=" bg-white sm:w-3/5 w-11/12 rounded-lg mt-10">
                  <div className="flex justify-center">
                    ...
                    <p className="text-3xl font-semibold co-black">
                      {item.question_no}
                    </p>
                  </div>
                  <div className="text-center">
                    <h2 className="text-lg sm:text-2xl font-bold co-black">
                      {item.question_name}
                    </h2>
                  </div>
                </div>
                <div className=" bg-white sm:w-3/5 w-11/12 flex flex-col text-center mt-8 rounded-lg items-center justify-center">
                  <div className="my-2 w-92 sm:w-92">
                    {item.select_type === "1" &&
                      item.select1
                        .split(",")
                        .filter((item1) => item1 !== "")
                        .map((item2, idx2) => (
                          ...
                          >
                            <input
                              checked={item.answer === item2}
                              onChange={(event) => {
                                changeQuestionStatus2(
                                  event.target.value,
                                  idx,
                                  item.select2.split(",")[idx2]
                                );
                              }}
                              className="mr-3 enabled:hover:border-gray-400 appearance-none checked:bg-gray-500 w-5 h-5 checked:bg-[length:1.5em_1.5em] focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-gray-500 selcolor-black co-black"
                              type="radio"
                              name="like"
                              value={item2}
                            />
                            <label
                              className={
                                item.error === false
                                  ? "text-base font-medium co-black"
                  ...
                  {item.select_type === "2" && (
                    <div className="my-2 w-11/12">
                      <div className="flex flex-row items-center justify-start">
                        <input
                          name="technical"
                          value={item.answer !== -1 ? item.answer : ""}
                          onChange={(event) => {
                            changeValue(event.target.value, idx);
                          }}
                          className={
                            item.error === false
                              ? "focus:border-sky-200	w-full	rounded border-gray-300 selcolor-black co-black"
                              : "focus:border-sky-200	w-full	rounded border-red-500 selcolor-black co-black"
                          }
                          type="text"
                          placeholder="回答を入力"
                        />
                  ...
                  <div className="flex flex-row items-center mt-5">
                    <button
                      onClick={() => {
                        item.select_type === "1"
                          ? nextQuestion(idx)
                          : nextQuestion1(idx, item.require);
                      }}
                      className="sm:w-36 w-80 bg-orange-400 focus:outline-none hover:bg-orange-500 hover:border-orange-400	text-white"
                      type="button"
                    >
                      次へ
                    </button>
                ...
                {item.question_no !== "Q1" && (
                  <div className="flex flex-row items-center mt-5">
                    <button
                      onClick={() => {
                        previousQuestion(item.previousId, idx);
                      }}
                      className="w-32 sm:w-36 co-black  bg-white focus:outline-none hover:border-gray-100 hover:bg-gray-100 mr-4"
                      type="button"
                    >
                      戻る
                    </button>
                    <button
                      ...
                      className="w-32 sm:w-36  bg-orange-400 focus:outline-none hover:bg-orange-500 hover:border-orange-400	text-white"
                      type="button"
                    >
                      次へ
                    </button>
          ...
          <main className="flex flex-col items-center">
            ...
            <div className="w-11/12 flex flex-col text-center mt-8 items-center justify-center">
              <div className="my-2 w-11/12 rounded-lg bg-white">
                <p className="text-sm	sm:text-lg font-medium co-black">
                  ご回答ありがとうございます!
                </p>
                <p className="text-sm	sm:text-lg font-medium co-black">
                  口コミのご協力お願いいたします。
                </p>
                <p className="mt-3 text-sm	font-medium co-black">
                  ※3秒後に自動的に口コミページに移動します。
                </p>
              </div>
            </div>
          </main>
          ...
          <main className="flex flex-col items-center">
            ...
            <div className=" w-11/12 flex flex-col text-center mt-8 items-center justify-center">
              <div className="my-2 w-11/12">
                ...
                <p className="text-sm	sm:text-lg font-medium co-black">
                  回答いただいた内容は、今後のより良いサービス改善に活用していきます。
                </p>
                <p className="text-sm	sm:text-lg font-medium co-black">
                  今後とも「{store_name}」をよろしくお願いいたします。
                </p>
              </div>
            </div>
          </main>
