...
function ManageStore() {
  ...
  const { store, isLoading, status, error } = useSelector(
    (store) => store.store
  );
  ...
  return (
    <Flowbite>
      ...
      <main className="flex flex-row width-1200 mx-auto my-0">
            ...
            <div className="px-8 dark:bg-gray-800">
              <div className="flex flex-row gap-4 items-center">
                ...
                <FloatingLabel
                  variant="outlined"
                  label="URLの名前"
                  value={store_url_name}
                  onChange={(e) => setStoreURLName(e.target.value)}
                />
                <FloatingLabel
                  className="w-72"
                  variant="outlined"
                  label="店舗Googleビジネスurl"
                  value={store_business_url}
                  onChange={(e) => setStoreBusinessURL(e.target.value)}
                />
                <Select
                  onChange={(e) => {
                    setGroup_id(e.target.value);
                  }}
                  className=" h-14 rounded border-gray-500"
                >
                  <option value=""></option>
              ...
              <table className="mt-2 ml-5 border-collapse	w-11/12">
                <thead>
                  <tr className="border border-slate-300 border-l-0 border-r-0">
                    ...
                    <th className="text-sm px-1 py-4 break-words">
                      調査調査url
                    </th>
                  ...
                  {store === null || store.length === 0 ? (
                    ...
                    store.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-2 py-2 text-sm break-all">
                          {item.store_url_name}
                        </td>
                        ...
                        <td className="px-2 py-2 text-sm flex flex-row">
                          ...
                          <a
                            href="#"
                            onClick={() =>
                              dispatch(deleteStore({ store_id: item.id }))
                            }
        ...
        <Modal
          show={edit_store_id !== "-1"}
          onClose={() => setEditStoreId("-1")}
        >
          ...
          <Modal.Body>
            <div className="space-y-6 flex flex-col">
              ...
              <div className="">
                ...
                <TextInput
                  value={edit_store_url_name}
                  onChange={(e) => {
                    setEditStoreURLName(e.target.value);
                  }}
                />
              </div>
              <div className="">
                ...
                <Select
                  value={edit_group_id}
                  onChange={(e) => {
                    setEdit_Group_id(e.target.value);
                  }}
                  className=" h-14 rounded border-gray-500"
                >
                  <option value=""></option>
              ...
              <div className="">
                ...
                <TextInput
                  value={edit_store_business_url}
                  onChange={(e) => {
                    setEditStoreBusinessURL(e.target.value);
                  }}
                />
              </div>
            </div>
              ...
              onClick={() => {
                dispatch(
                  createOrUpdate({
                    ...
                    store_group_id: edit_group_id,
                    store_name: edit_store_name,
                    store_url_name: edit_store_url_name,
                    store_business_url: edit_store_business_url,
                  })
                );
              }}
            >
